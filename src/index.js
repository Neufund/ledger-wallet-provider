...
inherits(LedgerWalletSubprovider, HookedWalletSubprovider)

function LedgerWalletSubprovider() {
	var self = this;
	LedgerWalletSubprovider.super_.call(self, {getAccounts: ()=>{}})
	
	self.path = "44'/60'/1337'";
	self.accounts = undefined
	self.scrambleKey = "w0w" // Hardcoded key for the Ledger Nano S
	self.ledger3 = new Ledger3(self.scrambleKey);
	self.ledger = new LedgerEth(self.ledger3);
	
	window.ledger = self.ledger;
	
	self.ledger.getAppConfiguration((config)=>{
	...
	self.getAccounts = (cb)=>{
		...
		const display = true;  // Ask for on-device confirmation
	...
	self.approveTransaction = (tx, cb)=>{
		console.log('approveTransaction', tx);
		// Approved!
		cb(undefined, true);
	}
	
	self.signTransaction = (txData, callback)=>{
	...
	self.signMessage = function(msgParams, callback) {
