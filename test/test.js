...
const ganacheAcc1 = isNode ? "0x7bd1e022bdfbd45d77913bec1582dc095cb5fa31" : "0x627d795782f653c8ea5e7a63b9cdfe5cb6846d9f";
const ganacheAcc2 = isNode ? "0x2e6d01625685281a1e3d10e4b41a61b4e6acb55f" : "0xf40011040398947b3c6b7532ed23fbc8c19c9654";

const rpcUrl = isNode ? "http://localhost:8545" : "https://localhost:8080/node/";
...
async function main () {
  ...
  console.log(`First account from ledger: ${ledgerDp0Acc0} on derivation path: ${config.dp0}`);
  ...
  console.log(`Second account from ledger: ${ledgerDp0Acc1} on derivation path: ${config.dp0}`);
  if (config.dp0Acc1.toLowerCase() !== ledgerDp0Acc1.toLowerCase()) {
    engine.stop();
    throw new Error(`Account dp1Acc1 mismatch when using ledger.MultipleAccounts`);
  }
  ...
  console.log(`First account from ledger: ${ledgerDp1Acc0} on derivation path: ${config.dp1}`);
  ...
  console.log(`Second account from ledger: ${ledgerDp1Acc1} on derivation path: ${config.dp1}`);
  ...
  console.log(web3.fromWei(await web3.eth.getBalanceAsync(ganacheAcc1), "ether").toString());
  console.log(web3.fromWei(await web3.eth.getBalanceAsync(ganacheAcc2), "ether").toString());
